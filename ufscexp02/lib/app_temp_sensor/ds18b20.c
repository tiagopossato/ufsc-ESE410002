...
bool ds18b20_setResolution(const DeviceAddress tempSensorAddresses[], int numAddresses, uint8_t newResolution)
{
	...
	ESP_LOGI(TAG, "Resolution: %d", newResolution);
...
void ds18b20_select(const DeviceAddress *address)
{
	...
	for (i = 0; i < 8; i++)
		ds18b20_write_byte(((uint8_t *)address)[i]);
}
...
void ds18b20_requestTemperatures()
{
	...
	while (!isConversionComplete() && ((esp_timer_get_time() / 1000ULL) - start < millisToWaitForConversion()))
		vPortYield();
}
...
float ds18b20_get_temp(void)
{
	if (init == 1)
	{
		...
		if (check == 1)
		{
			ds18b20_send_byte(SKIPROM); // Ignorar endereço(somente um sensor)
			ds18b20_send_byte(GETTEMP); // This command initiates a single temperature conversion.
			vTaskDelay(millisToWaitForConversion() / portTICK_RATE_MS);
			check = ds18b20_RST_PULSE();
